...
@torch.no_grad()
def extract(model, X, FC, idxs, need_graph, bs=128):
    model.eval(); R, S = [], []
    idxs = np.asarray(idxs)
    for lo in range(0, len(idxs), bs):
        b = MO.make_batch(X, FC, idxs[lo:lo + bs], need_graph)
        r, lg = model(b, None)
        R.append(r.numpy()); S.append(lg.numpy())
    return np.concatenate(R), np.concatenate(S)

# ------------------------------------------------------------------ train
def train_fold(arch, X, FC, y, tr, cfg, seed, log=None):
    ...
    for ep in range(1, max_ep + 1):
        ...
        for lo in range(0, len(perm), BATCH):
            ...
            opt.zero_grad()
            _, lg = model(MO.make_batch(X, FC, ii, need_graph), None)
            loss = lossf(lg, yt[ii])
            loss.backward()
        ...
        model.eval()
        with torch.no_grad():
            _, st = extract(model, X, FC, itr, need_graph)
            _, sv = extract(model, X, FC, iva, need_graph)
