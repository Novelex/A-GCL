...
#SBATCH --output=/users/3171356m/A-GCL/audit/s16/logs/ctrlu_%a.out
#SBATCH --error=/users/3171356m/A-GCL/audit/s16/logs/ctrlu_%a.err
...
n={'bnt':len(G.BNTU),'wgin':len(G.WGINU),'ctrlu':len(G.CTRLU)}['ctrlu']
exp=24
assert n==exp, f'FROZEN GRID CHANGED: ctrlu has {n} units, expected {exp}. STOP.'
"
...
trap _fwd USR1
/users/3171356m/A-GCL/.venv/bin/python -u $S/s16_worker.py ctrlu $SLURM_ARRAY_TASK_ID prod &
PY_PID=$!
