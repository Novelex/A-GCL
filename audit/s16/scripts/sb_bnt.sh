...
n={'bnt':len(G.BNTU),'wgin':len(G.WGINU),'ctrlu':len(G.CTRLU)}['bnt']
exp=72
assert n==exp, f'FROZEN GRID CHANGED: bnt has {n} units, expected {exp}. STOP.'
"
...
trap _fwd USR1
/users/3171356m/A-GCL/.venv/bin/python -u $S/s16_worker.py bnt $SLURM_ARRAY_TASK_ID prod &
PY_PID=$!
