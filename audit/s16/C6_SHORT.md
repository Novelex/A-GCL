...
A7 exists to bridge C6's tr_enc(610) scale to C2's full-tr(763) scale, which requires
that training-set size be the ONLY difference. It is not. The TRAINING RECIPE also
differs:
...
reference point: the SVM itself trained on tr_enc reads **0.7319** versus **0.7565** on
...
**HEADLINE:  delta = AUC(fused at chosen alpha) - svm_tr_enc**
