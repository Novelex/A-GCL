...
OUT = "/users/3171356m/A-GCL/audit/s17/runs/tangent2/"
...
MEMDIR = os.environ.get("TAN_CACHE", "/tmp/claude-102000043/-users-3171356m-A-GCL/"
                        "a9e2c01c-7bf7-4c3e-b518-73a2642d296c/scratchpad/tan2_cache")
...
if abs(res["L0_FC_flat|cv"] - 0.7565) > 5e-4:
    log("*** GATE L0 failed — STOP"); aj(dict(halted="L0", res=res), OUT+"TANGENT2.json"); sys.exit(3)
if abs(res["L1_FCamp_flat|cv"] - 0.7261) > 5e-4:
    log("*** GATE L1 failed — STOP"); aj(dict(halted="L1", res=res), OUT+"TANGENT2.json"); sys.exit(3)
...
for mode, folds in (("cv", C.OUTER), ("loso", loso_folds)):
    ...
    alpha_per_fold[mode] = alphas
    log(f"RUN T1_FC_tangent  {mode:4s} AUC {res[f'T1_FC_tangent|{mode}']:.4f}  alphas {alphas}")
    log(f"RUN T2_FCamp_tang. {mode:4s} AUC {res[f'T2_FCamp_tangent|{mode}']:.4f}  (same alphas)")
...
aj(dict(implementation="scipy log-Euclidean + shrinkage (1-a)C+aI; alpha in "
        ...
        results=res, bootstrap=boots, alpha_per_fold=alpha_per_fold,
        ...
        log=log_lines), OUT + "TANGENT2.json")
np.savez_compressed(OUT + "TANGENT2_oof.npz", y=y, **oofs)
