#!/bin/bash
#SBATCH --job-name=s17swB
#SBATCH --partition=gpu-l40s,gpu-h100
#SBATCH --array=0-999%60
#SBATCH --cpus-per-task=4
#SBATCH --mem=12G
#SBATCH --time=12:00:00
#SBATCH --output=/users/3171356m/A-GCL/audit/s17/logs/sweep/B_%A_%a.out
#SBATCH --error=/users/3171356m/A-GCL/audit/s17/logs/sweep/B_%A_%a.err
# Stage B: 3,132 units (29 folds x 6 inputs x 18 chunks of 30 configs) packed into 1000 tasks; 4 single-thread trainings per task.
set -euo pipefail
...
"
export SWEEP_NTASK_B=1000 SWEEP_NPAR=4
test -f /users/3171356m/A-GCL/audit/s17/runs/sweep/A/AGG_A.json || { echo "AGG_A.json absent - Stage A not aggregated. STOP."; exit 3; }
$PY -c "import json; a=json.load(open('/users/3171356m/A-GCL/audit/s17/runs/sweep/A/AGG_A.json')); assert a['lab'].get('gate_pass') is True, 'LAB GATE FAILED IN STAGE A. STOP.'; assert all(a[p].get('complete') for p in ('lab','site','loso')), 'STAGE A INCOMPLETE. STOP.'"
$PY -u $S/taskB.py $SLURM_ARRAY_TASK_ID
